...
{
    public class StopApp : IStopsApp
    {
        private readonly IStopInfra _stopInfra;
        private readonly IMapper _mapper;

        public StopApp(IStopInfra stopInfra, IMapper mapper)
        {
            _stopInfra = stopInfra;
            _mapper = mapper;
        }


        public async Task<IEnumerable<StopLocation>> GetStopsLocations(string latitude, string longitude, string data, string hora)
        {

            var response = await _stopInfra.GetStops(latitude, longitude, data, hora);
            var result = await ParseResponse(response, latitude, longitude);
            return result;
